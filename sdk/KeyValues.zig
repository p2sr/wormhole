...
pub fn init(name: [*:0]const u8) !*KeyValues {
    const mem = @ptrCast(?*KeyValues, keyValuesSystem().allocKeyValuesMemory(@sizeOf(KeyValues))) orelse return error.OutOfMemory;
    ...
    mem.* = KeyValues{
        .key_name = .{
            .name = @truncate(u24, case_insens_key),
            .case_sens_1 = @truncate(u8, case_sens_key),
        },
        .key_name_case_sens_2 = @truncate(u16, case_sens_key >> 8),
    };
...
pub fn setString(self: *KeyValues, key: [*:0]const u8, val: []const u8) !void {
    if (try self.findKey(key, true)) |kv| {
        kv.val_str = @ptrCast(?[*:0]u8, keyValuesSystem().allocKeyValuesMemory(val.len + 1)) orelse return error.OutOfMemory;
